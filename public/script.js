        ...
        const response = await new Promise((resolve, reject) => {
            socket.emit('get_app_data', (response) => {
                if (response.status === 'success') {
                    ...
                    reject(new Error(response.message || 'Unknown error fetching app data.'));
                }
            });
        });
    ...
    return new Promise((resolve, reject) => {
        socket.emit('get_event', week, (response) => {
            ...
            if (response.status === 'error') {
                console.error(`Failed to fetch event for week ${week}:`, response.message);
                alert(`Could not load the next event: ${response.message}`);
                prefetchedEvent = null;
                reject(new Error(response.message));
            } else {
                prefetchedEvent = response.data;
                resolve();
            }
        });
    });
...
function showFinalSplash() {
    ...
    splashHTML += `<hr style="border-color: var(--tertiary-bg); margin: 20px 0;">
                   <div id="end-game-buttons">
                       <p style="font-size:12px; color:var(--grey-accent);">Your session data has been submitted.</p>
                       <button id="home-button" class="modal-button">Return to Home</button>
                   </div>`;
